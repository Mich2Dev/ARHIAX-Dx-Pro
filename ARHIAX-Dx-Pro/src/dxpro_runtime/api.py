...
from .logging_config import METRICS, StructuredLoggingMiddleware
...
def create_app(config: RuntimeConfig | None = None) -> FastAPI:
    ...
    def healthz() -> dict[str, Any]:
        ledger_ok = True
        try:
            ledger_ok = runtime.ledger.verify().get("valid", True)
        except Exception:
            ledger_ok = False
        return {
            "status": "ok",
            ...
            "opa_mode": runtime.policy_engine.mode,
            "ledger_ok": ledger_ok,
        }
    ...
    @app.get("/v1/metrics", dependencies=protected)
    ...
    @app.get("/v1/exports/{case_id}/{filename}", dependencies=protected)
    def download_export(case_id: str, filename: str) -> Any:
        from fastapi.responses import FileResponse
        # Prevent path traversal
        if "/" in filename or "\\" in filename or ".." in filename:
            raise HTTPException(status_code=400, detail={"error": "invalid_filename"})
        file_path = config.export_root / case_id / filename
        if not file_path.exists() or not file_path.is_file():
            raise HTTPException(
                status_code=404,
                detail={"error": "export_not_found", "case_id": case_id, "filename": filename},
            )
    ...
    def grammar_lint(request: GrammarLintRequest) -> dict[str, Any]:
        if not request.text or not request.text.strip():
            raise HTTPException(
                status_code=400,
                detail={"error": "text_empty", "message": "El texto no puede estar vacío."},
            )
        if request.audience not in ("internal", "client", "technical", "executive"):
            raise HTTPException(
                status_code=400,
                detail={"error": "invalid_audience", "message": f"Audiencia inválida: {request.audience}"},
            )
        exceptions = [
            GrammarException(**e) for e in request.exceptions if e.get("reason", "").strip()
        ]
