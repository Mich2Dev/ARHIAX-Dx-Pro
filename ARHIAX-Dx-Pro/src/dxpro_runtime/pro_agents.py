...
class GovernedAgent:
    ...
    def execute(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            # LLM or build error: log to ledger, return stub with llm_error flag
            error_msg = str(exc)
            try:
                from .logging_config import log_llm_error
                log_llm_error(
            ...
            artifact = self._build_stub_fallback(artifact_payload, error_msg)
    ...
    def _build_stub_fallback(self, payload: dict[str, Any], error: str) -> dict[str, Any]:
        """Return a minimal stub artifact when _build_artifact raises."""
        return {
            "artifact_type": getattr(self, "_artifact_type", "agent_artifact"),
            "llm_mode": "stub",
            "llm_error": True,
            "error": error,
        }
...
class RgcAgent(GovernedAgent):
    ...
    def _build_artifact(self, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        # Detect stub mode when credentials are missing
        import os as _os
        openalex_email = _os.getenv("OPENALEX_CONTACT_EMAIL")
        lens_token = _os.getenv("LENS_API_TOKEN")
        stub_mode = not openalex_email and not lens_token
        ...
        pack["stub_mode"] = stub_mode
        if search_errors:
            pack["search_errors"] = search_errors

        # Log to ledger: result count and query (no token)
        result_count = len(pack.get("hypotheses") or [])
        self.runtime.ledger.append(
            {
                "trace_id": payload.get("trace_id", ""),
                "subject": self.subject,
                "event_type": "rgc_search",
                "agent": self.component,
                "domain": domain,
