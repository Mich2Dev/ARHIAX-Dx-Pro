...
class ApiKeyAuth:
    ...
    def __call__(self, presented: str | None) -> str | None:
        ...
        if presented is None:
            from .logging_config import log_auth_rejected
            log_auth_rejected(fingerprint="(no key)", reason="missing_api_key")
            raise HTTPException(
                status_code=401,
        ...
        from .logging_config import log_auth_rejected
        log_auth_rejected(fingerprint=self.fingerprint(presented), reason="invalid_api_key")
